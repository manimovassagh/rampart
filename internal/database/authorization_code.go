...
func (db *DB) StoreAuthorizationCode(ctx context.Context, code, clientID string, userID, orgID uuid.UUID, redirectURI, codeChallenge, scope, nonce string, expiresAt time.Time) error {
	...
	_, err := db.Pool.Exec(ctx, `
		...
		hash[:], clientID, userID, orgID, redirectURI, codeChallenge, scope, nonce, expiresAt)
	if err != nil {
...
func (db *DB) ConsumeAuthorizationCode(ctx context.Context, code string) (*model.AuthorizationCode, error) {
	...
	row := db.Pool.QueryRow(ctx, `
		...
		RETURNING id, code_hash, client_id, user_id, org_id, redirect_uri, code_challenge, scope, nonce, used, expires_at, created_at`,
		hash[:])

	var ac model.AuthorizationCode
	err := row.Scan(&ac.ID, &ac.CodeHash, &ac.ClientID, &ac.UserID, &ac.OrgID,
		&ac.RedirectURI, &ac.CodeChallenge, &ac.Scope, &ac.Nonce, &ac.Used, &ac.ExpiresAt, &ac.CreatedAt)
	if err != nil {
